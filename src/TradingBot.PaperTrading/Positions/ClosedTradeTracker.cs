...
/// <summary>
/// Dekoriert einen echten <see cref="IPositionManager"/> und rekonstruiert abgeschlossene
/// Round-Turn-Trades (flat → flat bzw. Flip) für die Paper-Session. Nutzt AUSSCHLIESSLICH
/// die kumulativen Realized-Werte des inneren PositionManagers (Deltas) – keine eigene
/// Netting-Mathematik, daher keine Doppelzählung: Σ Trade-Fees = gezahlte Fees und
/// Trade.NetPnL = Trade.GrossPnL − Fees.TotalFees.
/// Hat KEINE Execution-Referenz und sendet niemals Orders.
/// </summary>
public sealed class ClosedTradeTracker : IPositionManager
{
    ...
    private readonly List<PaperClosedTrade> _trades = new();
    private readonly object _sync = new();

    public ClosedTradeTracker(IPositionManager inner)
        => _inner = inner ?? throw new ArgumentNullException(nameof(inner));

    /// <summary>Wird bei jedem abgeschlossenen Trade aufgerufen (DailyState + Journal).</summary>
    public Action<PaperClosedTrade>? TradeClosed { get; set; }

    public IReadOnlyList<PaperClosedTrade> Trades
    {
    ...
    private static void OpenTrade(OpenState state, FillEvent fill, Position after, Position? baselineFrom)
    {
        ...
        state.BaselineFees = baselineFrom?.Fees ?? FeeBreakdown.Zero;
    }

    private void CloseTrade(OpenState state, FillEvent fill, Position after)
    {
        var trade = new PaperClosedTrade
        {
            Symbol = fill.Symbol,
            ...
            Fees = after.Fees - state.BaselineFees,
    ...
    private sealed class OpenState
    {
        ...
        public FeeBreakdown BaselineFees = FeeBreakdown.Zero;
    }
}
